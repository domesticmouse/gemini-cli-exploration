...
export class Project extends EventTarget implements IDisposable {
    ...
    async #load_file(filename: string): Promise<KicadPCB | KicadSch | void | null | undefined> {
    ...
    async #load_doc(
        ...
        filename: string,
    ): Promise<KicadPCB | KicadSch | null | undefined> {
    ...
    async #load_meta(filename: string): Promise<void> {
        const text = await this.#get_file_text(filename);
        const data = JSON.parse(text);
        this.settings = ProjectSettings.load(data);
    }

    async #get_file_text(filename: string): Promise<string> {
        return await (await this.#fs.get(filename)).text();
    }

    #determine_schematic_hierarchy(): void {
    ...
    public *files(): Generator<KicadPCB | KicadSch | null | undefined> {
        yield* this.#files_by_name.values();
    }

    public file_by_name(name: string): KicadSch | KicadPCB | null | undefined {
        return this.#files_by_name.get(name);
    }

    public *boards(): Generator<KicadPCB> {
    ...
    public get has_boards(): boolean {
        return length(this.boards()) > 0;
    }

    public *schematics(): Generator<KicadSch> {
        for (const value of this.#files_by_name.values()) {
    ...
    public get has_schematics(): boolean {
        return length(this.schematics()) > 0;
    }

    public *pages(): Generator<ProjectPage> {
        yield* this.#pages_by_path.values();
    }

    public get first_page(): ProjectPage | undefined{
        return first(this.pages());
    }

    public get root_schematic_page(): ProjectPage | undefined {
    ...
    public get active_page(): ProjectPage | null {
...
export class ProjectPage {
    ...
    get document(): KicadPCB | KicadSch | null | undefined {
